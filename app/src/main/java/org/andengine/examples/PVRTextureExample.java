...
public class PVRTextureExample extends SimpleBaseGameActivity {
	...
	public EngineOptions onCreateEngineOptions() {
		Toast.makeText(this, "The lower houses use MipMaps!", Toast.LENGTH_SHORT).show();
		Toast.makeText(this, "Click the top half of the screen to zoom in or the bottom half to zoom out!"
	...
	public Scene onCreateScene() {
		...
		final float centerX = CAMERA_WIDTH / 2f;
		final float centerY = CAMERA_HEIGHT / 2f;
		...
		scene.setOnSceneTouchListener(new IOnSceneTouchListener() {
			...
			public boolean onSceneTouchEvent(final Scene pScene, final TouchEvent pSceneTouchEvent) {
				if (pSceneTouchEvent.isActionDown() || pSceneTouchEvent.isActionMove()) {
					if(pSceneTouchEvent.getY() < CAMERA_HEIGHT / 2f) {
